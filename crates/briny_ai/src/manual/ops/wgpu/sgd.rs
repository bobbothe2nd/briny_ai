use super::{GPU_CONTEXT, GpuFailure, SGD_BIND_GROUP_LAYOUT, SGD_PIPELINE};
use crate::manual::{
    TensorFloat,
...
mod tests {
    use super::*;
    use crate::manual::{ops::wgpu::array_from_slice, tensors::TensorOps};
    ...
    fn wgpu_sgd_basic_update() {
        ...
        for (&u, &e) in updated.iter().zip(expected.iter()) {
            assert_relative_eq!(u as f32, e, epsilon = 1e-5);
        }
    }
    ...
    fn wgpu_sgd_zero_gradient() {
        ...
        for (&u, &e) in updated.iter().zip(expected.iter()) {
            assert_relative_eq!(u as f32, e, epsilon = 1e-6);
        }
    }
