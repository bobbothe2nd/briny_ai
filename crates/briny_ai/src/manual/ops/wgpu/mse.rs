...
use super::{Box, GPU_CONTEXT, GpuFailure, MSE_LOSS_BIND_GROUP_LAYOUT, MSE_LOSS_PIPELINE};
use crate::manual::{
    TensorFloat,
...
mod tests {
    ...
    fn wgpu_mse_loss_forward_basic() {
        let pred_data = [0.0f64, 0.5, 1.0];
        let target_data = [0.0f64, 1.0, 1.0];
        ...
        assert_abs_diff_eq!(loss, 0.0833333, epsilon = 1e-4);
    }
    ...
    fn wgpu_mse_loss_backward_gradient_shape_and_values() {
        ...
        let expected_grads: Vec<f64> = pred_data
            .iter()
