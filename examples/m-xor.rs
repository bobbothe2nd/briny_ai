...
use briny_ai::nn::tensors::IntoWithGrad;
use briny_ai::nn::tensors::{Tensor, TensorOps};
...
fn main() {
    ...
    for epoch in 0..epochs {
        ...
        for i in 0..4 {
            ...
            let (a1, _) = relu(&z1.with_grad());
            let (z2, _) = matmul(&a1.with_grad(), &w2);
...
fn main() {
    ...
    for epoch in 0..epochs {
        ...
        for &i in &idx {
            ...
            let (a1, back_relu) = relu(&z1_wrapped);
            let a1_wrapped = a1.with_grad();
            let (z2, back2) = matmul::<4, 8, 2, 2>(&a1_wrapped, &w2);
            let prediction = z2.with_grad();
        ...
        for i in 0..4 {
            ...
            let (z1, _) = matmul::<2, 8, 4, 2>(&xg, &w1);
            let (a1, _) = relu(&z1.with_grad());
            let (z2, _) = matmul::<4, 8, 2, 2>(&a1.with_grad(), &w2);
