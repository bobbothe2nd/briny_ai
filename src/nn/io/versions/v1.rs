...
pub fn save_tensors<T: TensorGrad<U> + TensorOps<U>, U: Pod + Copy + IntermediateFp>(
    ...
    let len = 8
        ...
        + tensors
            .iter()
            .map(|t| 8 + t.shape().len() * 8 + t.data().len() * 8 + 4)
            .sum::<usize>()
        + 4;
...
pub fn load_tensors<T: TensorGrad<U> + TensorOps<U>, U: Pod + Copy + IntermediateFp>(
    ...
    let count = u64::from_le_bytes(vec[8..16].try_into().map_err(|_| unreachable!())?);
...
pub fn deserialize_tensors<T: TensorGrad<U> + TensorOps<U>, U: Pod + Copy + IntermediateFp>(
    ...
    // tensor count
    let count = u64::from_le_bytes(buf[8..16].try_into().map_err(|_| unreachable!())?) as usize;
...
pub fn serialize_tensor<T: TensorGrad<U> + TensorOps<U>, U: Pod + Copy + IntermediateFp>(
    ...
    let t_len = tensor.len() * size_of::<f64>();
    buf[idx..idx + t_len].copy_from_slice(slice_to_bytes(
        &tensor
            .data()
            .iter()
            .map(|x| x.into_f64())
...
pub fn deserialize_tensor<T: TensorGrad<U> + TensorOps<U>, U: Pod + Copy + IntermediateFp>(
    ...
    let ndim = u64::from_le_bytes(buf8.try_into().map_err(|_| unreachable!())?) as usize;
    ...
    for _ in 0..ndim {
        ...
        shape.push(u64::from_le_bytes(
            buf8.try_into().map_err(|_| unreachable!())?,
        ));
    }
    ...
    for _ in 0..size {
        ...
        data.push(U::from_f64(f64::from_le_bytes(
            buf8.try_into().map_err(|_| unreachable!())?,
        )));
    }
...
mod tests {
    ...
    fn save_load_roundrip() {
        ...
        save_tensors("checkpoints/test/v1.bpat", &original).unwrap();

        let loaded: Vec<VecTensor<f32>> = load_tensors("checkpoints/test/v1.bpat").unwrap();
